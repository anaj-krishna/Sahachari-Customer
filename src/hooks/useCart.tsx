...
import { getCart, placeOrder, removeCartItem, updateCartItemQuantity } from "../services/orders.api";
...
export function useCart() {
  ...
  const updateQuantityMutation = useMutation({
    ...
    onMutate: ({ itemId }) => setUpdatingItems((prev) => new Set(prev).add(itemId)),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["cart"] }),
    onSettled: (_, __, { itemId }) => setUpdatingItems((prev) => {
  ...
  const removeItemMutation = useMutation({
    ...
    onMutate: (itemId: string) => setUpdatingItems((prev) => new Set(prev).add(itemId)),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["cart"] }),
    onSettled: (_, __, itemId) => setUpdatingItems((prev) => {
      const next = new Set(prev);
      next.delete(itemId);
  ...
  return {
    ...
    showSuccessModal, setShowSuccessModal, address, setAddress,
    handleQuantityChange: (id: string, cur: number, d: number) => {
      const next = cur + d;
      if (next >= 1) updateQuantityMutation.mutate({ itemId: id, quantity: next });
    },
